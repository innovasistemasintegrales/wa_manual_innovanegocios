...
let registrosMostrados = 0; // Contador de registros actualmente en la tabla de últimos incidentes
...
function actualizarFooterUltimosIncidentes() {
    ...
    mostrandoEl.textContent = registrosMostrados;
    totalEl.textContent = totalRegistros;
      ...
      if (valido === true) {
          if (telefono.length == 9) {
              if (dni.length == 8) {
                  ...
                  // socket.emit('/tecnico/registrarUsuario', nuevoUsuario);
...
//? Escuchar el clic en el botón de "Confirmar" dentro del modal de confirmación reutilizable
